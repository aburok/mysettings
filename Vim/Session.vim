...
vnoremap  "+x
nnoremap   /
nnoremap <silent> '? :call signature#ListLocalMarks()
nnoremap <silent> '[ :call signature#GotoMark( "prev", "line", "alpha" )
nnoremap <silent> '] :call signature#GotoMark( "next", "line", "alpha" )
...
vmap ,fp S<pre>$movit:s/<p>\(.\+\)<\/p>/\1/g/Listing
nmap ,fb /\_s{\_s\Sjvi{>
nmap ,fc /\(\s\+\){\_s\1\S$vi{>
nmap ,fn /<pre>\_s\s\+$vit<
nmap ,fi /{vi{>
nnoremap ,nt :sp note:privTodo
nnoremap ,np :sp note:todo
nnoremap ,n3 :sp note:3things
nnoremap ,it "=strftime("%c")p
nnoremap ,tm jmlkdd/^DONE TASKS$/Ip$a - ,it'l
nnoremap ,tt A - TODO
nnoremap ,td V:s/TODO/DONE/I
nmap ,w :w!
map ,s? z=  " Suggest correct word for word under the cursor
map ,sp [s " Go to previous misspelled word
map ,sn ]s " Go to next misspelled word
...
vmap [% [%m'gv``
nnoremap <silent> [= :call signature#GotoMarker( "prev", "any" )
nnoremap <silent> [- :call signature#GotoMarker( "prev", "same" )
nnoremap <silent> [` :call signature#GotoMark( "prev", "spot", "pos" )
nnoremap <silent> [' :call signature#GotoMark( "prev", "line", "pos" )
vmap ]% ]%m'gv``
nnoremap <silent> ]= :call signature#GotoMarker( "next", "any" )
nnoremap <silent> ]- :call signature#GotoMarker( "next", "same" )
nnoremap <silent> ]` :call signature#GotoMark( "next", "spot", "pos" )
nnoremap <silent> ]' :call signature#GotoMark( "next", "line", "pos" )
nnoremap <silent> `[ :call signature#GotoMark( "prev", "spot", "alpha" )
nnoremap <silent> `] :call signature#GotoMark( "next", "spot", "alpha" )
vnoremap ` :normal @a
nnoremap ` @a
vmap a% [%v]%
...
nmap ys <Plug>Ysurround
nnoremap <silent> zk mtO`t
nnoremap <silent> zj mto`t
nmap z" mtciw"""`t
nmap z' mtciw'"'`t
nmap z[ mtciw["]`t
nmap z( mtciw(")`t
...
inoremap  u
cmap  +
inoremap ,it =strftime("%c")
nmap ê my:m+`y
imap jj 
nmap ë my:m-2`y
inoremap <silent> { {}O
iabbr lol Lought Out Loud
iabbr imho In My Humble Opinion
iabbr brb Be Right Back
iabbr wtf What The Fuck
iabbr rtfm Read The Fine Manual
iabbr cwl Console.WriteLine
let &cpo=s:cpo_save
unlet s:cpo_save
set autoindent
set autoread
...
set nrformats=hex
set ruler
set runtimepath=~\\.vim\\bundle\\vundle,~\\.vim\\bundle\\nerdtree,~\\.vim\\bundle\\vim-signature,~\\.vim\\bundle\\vim-ps1,~\\.vim\\bundle\\vim-airline,~\\.vim\\bundle\\vim-fugitive,~\\.vim\\bundle\\vim-easymotion,~\\.vim\\bundle\\camelcasemotion,~\\.vim\\bundle\\vim-javascript,~\\.vim\\bundle\\vim-surround,~\\.vim\\bundle\\mru,~\\.vim\\bundle\\vim-misc,~\\.vim\\bundle\\vim-notes,~\\.vim\\bundle\\sessionman.vim,~\\.vim\\bundle\\nerdcommenter,~\\.vim\\bundle\\vim-autoclose,~\\.vim\\bundle\\vim-fugitive,~\\.vim\\bundle\\vim-sensible,~\\.vim\\bundle\\vim-easymotion,~\\.vim\\bundle\\neocomplcache.vim,~\\.vim\\bundle\\vim-addon-mw-utils,~\\.vim\\bundle\\tlib_vim,~\\.vim\\bundle\\vim-snipmate,~\\.vim\\bundle\\vim-snippets,~\\.vim\\bundle\\Conque-Shell,~\\.vim\\bundle\\ctrlp.vim,~\\.vim\\bundle\\sparkup\\vim\\,~/vimfiles,C:\\Program\ Files\ (x86)\\Vim/vimfiles,C:\\Program\ Files\ (x86)\\Vim\\vim74,C:\\Program\ Files\ (x86)\\Vim/vimfiles/after,~/vimfiles/after,C:\\Dropbox\\mysettings\\vim\\vundle,C:\\Dropbox\\mysettings\\vim,~\\.vim\\bundle\\vundle/after,~\\.vim\\bundle\\nerdtree/after,~\\.vim\\bundle\\vim-signature/after,~\\.vim\\bundle\\vim-ps
set scrolloff=1
set shiftround
set shiftwidth=4
set showmatch
...
silent only
cd C:\Dropbox\mysettings\vim
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +21 \Dropbox\mysettings\vim\_vsvimrc
badd +1 \Dropbox\mysettings\vim\_vimrc
badd +130 \Dropbox\mysettings\AutoHotKey\shortcuts\resharper.help
badd +44 \Dropbox\mysettings\vim\_vundle
badd +30 [Vundle]\ Installer
badd +0 \Dropbox\mysettings\vim\_vimrc.plugins
argglobal
silent! argdel *
edit \Dropbox\mysettings\vim\_vimrc.plugins
...
exe 'vert 1resize ' . ((&columns * 10 + 110) / 221)
exe '2resize ' . ((&lines * 26 + 27) / 54)
exe 'vert 2resize ' . ((&columns * 69 + 110) / 221)
exe '3resize ' . ((&lines * 25 + 27) / 54)
exe 'vert 3resize ' . ((&columns * 69 + 110) / 221)
exe 'vert 4resize ' . ((&columns * 70 + 110) / 221)
exe 'vert 5resize ' . ((&columns * 69 + 110) / 221)
argglobal
enew
file NERD_tree_1
let s:cpo_save=&cpo
...
setlocal tabstop=4
setlocal tags=C:/Dropbox/mysettings/.git/nerdtree.tags,C:/Dropbox/mysettings/.git/tags,./tags,tags
...
setlocal tabstop=4
setlocal tags=C:/Dropbox/mysettings/.git/vim.tags,C:/Dropbox/mysettings/.git/tags,./tags,tags
...
wincmd w
argglobal
edit \Dropbox\mysettings\AutoHotKey\shortcuts\resharper.help
setlocal keymap=
setlocal noarabic
setlocal autoindent
setlocal balloonexpr=
...
setlocal tabstop=4
setlocal tags=C:/Dropbox/mysettings/.git/tags,./tags,tags
setlocal textwidth=0
setlocal thesaurus=
setlocal noundofile
setlocal undolevels=-123456
...
setlocal wrap
setlocal wrapmargin=0
let s:l = 130 - ((0 * winheight(0) + 12) / 25)
if s:l < 1 | let s:l = 1 | endif
exe s:l
normal! zt
130
...
wincmd w
argglobal
edit \Dropbox\mysettings\vim\_vimrc
vnoremap <buffer> <silent> [" :exe "normal! gv"|call search('\%(^\s*".*\n\)\%(^\s*"\)\@!', "bW")
nnoremap <buffer> <silent> [" :call search('\%(^\s*".*\n\)\%(^\s*"\)\@!', "bW")
vnoremap <buffer> <silent> [] m':exe "normal! gv"|call search('^\s*endf*\%[unction]\>', "bW")
nnoremap <buffer> <silent> [] m':call search('^\s*endf*\%[unction]\>', "bW")
...
wincmd w
argglobal
edit \Dropbox\mysettings\vim\_vsvimrc
vnoremap <buffer> <silent> [" :exe "normal! gv"|call search('\%(^\s*".*\n\)\%(^\s*"\)\@!', "bW")
nnoremap <buffer> <silent> [" :call search('\%(^\s*".*\n\)\%(^\s*"\)\@!', "bW")
vnoremap <buffer> <silent> [] m':exe "normal! gv"|call search('^\s*endf*\%[unction]\>', "bW")
nnoremap <buffer> <silent> [] m':call search('^\s*endf*\%[unction]\>', "bW")
...
setlocal wrap
setlocal wrapmargin=0
let s:l = 21 - ((15 * winheight(0) + 26) / 52)
if s:l < 1 | let s:l = 1 | endif
exe s:l
normal! zt
21
